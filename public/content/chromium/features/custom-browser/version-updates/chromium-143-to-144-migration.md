...
| New-layout-implementation coverage for other custom containers | Not fully audited | `kTabbedBrowserUseNewLayout`'s default flip only surfaced the bottom-bar gap because it was reported after manual testing. Other fork-added containers wired only into `BrowserViewLayoutImplOld` (sidebar, vertical tab bar) were not proactively re-audited against `BrowserViewTabbedLayoutImpl` this round — worth a deliberate pass rather than waiting for the next bug report. |
