...
| `CustomLayoutContainers()` vs. the new `main_container_` concept | **Fixed in v1.9.1** | Confirmed as a real bug, not just a theoretical risk: `contents_container_`'s coordinates weren't adjusted for its new `main_container_` parent (black gap at the top of custom WebUI pages), the reinstated call site passed a height where a bottom Y-coordinate was expected (sidebar/vertical-tab-bar didn't reach the window's bottom edge), and `main_container_` spanned the sidebar's screen region while sitting on top of it in hit-testing order (swallowed sidebar clicks). See the [1.9.1 changelog entry](../changelog#191--2026-08-30) for the fix. |
