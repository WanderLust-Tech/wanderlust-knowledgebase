...
- **`CustomSearchProvider`** (`custom_search_provider.{h,cc}`) — the RSS-in-omnibox provider. This existed as dead code for a long time: it was added to the default provider bitmask (`TYPE_CUSTOM_SEARCH`) but never actually instantiated in `AutocompleteController`, and its matching logic was commented-out pseudocode referencing undefined symbols from an unrelated ancestor codebase. Both bugs were fixed in 1.8.16, but that fix introduced a real layering violation — reaching into `chrome/browser`-layer `RSSService`/`RSSServiceFactory`/`ChromeAutocompleteProviderClient::GetProfile()` directly from `components/omnibox/browser`, via an illegal `static_cast<ChromeAutocompleteProviderClient*>` — which only surfaced as an undefined-symbol link failure at full-browser link time (deferred at the time; see the 1.8.17 changelog entry). **Fixed properly in 1.8.29**: two new components/-safe virtuals, `AutocompleteProviderClient::IsRSSOmniboxSearchEnabled()` and `GetRSSOmniboxCandidates()` (default to `false`/empty, matching the base class's existing convention for optional embedder-specific accessors like `GetDocumentSuggestionsService()`), let `ChromeAutocompleteProviderClient` bridge the real RSS lookup from the chrome/ layer, so `CustomSearchProvider` never touches `Profile`/`RSSService` or any concrete client subclass directly. Matches typed text against subscribed RSS feed items' titles, gated behind `IsRSSOmniboxSearchEnabled()`, using the modern `FindTermMatches`/`ClassifyTermMatches` highlighting API.
