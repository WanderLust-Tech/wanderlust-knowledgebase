...
| Routing | `chrome://sidebar/bookmarks` / `…/history` / `…/rss` / `…/notes` / `…/ntp-settings` / `…/recently-closed` — `App.tsx` reads `window.location.pathname` and renders the matching page component |
...
| DOM handler | [`SidebarDOMHandler`](../../src/custom/browser/ui/webui/sidebar/sidebar_dom_handler.cc) — exposes RSS-read IPCs (`requestFeedList`, `requestFeedContent`, plus `readerFeedsChanged` / `readerUnreadChanged` listeners — same names as `ReaderDOMHandler` so the shared `RSSImpl` backend serves both), bookmark IPCs (`requestBookmarkTree` + `bookmarksChanged` listener), history IPCs (`requestHistory`, `removeHistoryEntry`, `historyChanged` listener), notes IPCs (`notesGetActiveUrl`, `notesGetForUrl`, `notesList`, `notesSave`, `notesDelete`), NTP settings IPCs (`ntpGetSettings`, `ntpSaveSettings`, `ntpGetCurrentLayout`), Recently Closed / Open Tabs IPCs (`requestRecentlyClosed`, `restoreClosedItem`, `requestOpenTabs`, `switchToOpenTab`), and a shared `openUrl` write that navigates the active browser tab |
| React bundle | [`//custom/components/custom_sidebar/`](../../src/custom/components/custom_sidebar/) — `main.tsx` → `App.tsx` → one of `pages/{RssPage,BookmarksPage,HistoryPage,NotesPage,NtpSettingsPage,RecentlyClosedPage}.tsx` |
| Native integration | [`SidebarContainerView`](../../src/custom/browser/ui/views/frame/sidebar_container_view.cc) loads `kChromeUISidebarBookmarksURL` / `…HistoryURL` / `…RssURL` / `…NotesURL` / `…NtpSettingsURL` / `…RecentlyClosedURL` into its embedded `WebView` when a top-pane button is pressed (or `ShowPanel()` is called programmatically) |
...
| `App.tsx` | Reads `window.location.pathname`, picks `<RssPage/>`, `<BookmarksPage/>`, `<HistoryPage/>`, `<NotesPage/>`, `<NtpSettingsPage/>`, or `<RecentlyClosedPage/>` |
| `types.ts` | `Feed`, `Group`, `FeedItem`, `FeedListResponse`, `SidebarRoute` (`'bookmarks' \| 'history' \| 'rss' \| 'notes' \| 'ntp-settings' \| 'agent' \| 'recently-closed'`), `PageNote`, `RecentlyClosedItem`, `OpenTab` |
...
| `pages/RecentlyClosedPage.tsx` | Two live sections: **Open Tabs** (every tab across every window of this profile, not just the sidebar's own window — clicking activates it) and **Recently Closed** (tabs + windows from `TabRestoreService` — clicking restores it; window entries show a tab-count badge). Same-device only |
...
| `requestRecentlyClosed` | — | `RecentlyClosedItem[]` — `{ title, url, timestampMs, isWindow, tabCount, sessionId }`, ported from `RemoteNtpServiceImpl::GetRecentSessions()`. `TAB` entries → single-tab item (`tabCount: 1`); `WINDOW` entries use the selected tab's nav as the representative title/url; `GROUP` entries are skipped |
| `requestOpenTabs` | — | `OpenTab[]` — `{ title, url, tabId, windowId, isActive }` across every open `Browser` window of this profile (not just the window hosting the sidebar), ported from `RemoteNtpServiceImpl::GetOpenTabs()` |
...
| `restoreClosedItem` | `[sessionId]` — `TabRestoreService::RestoreEntryById()` into the `LiveTabContext` of the browser window hosting the sidebar (`BrowserLiveTabContext::FindContextWithID`), ported from `RemoteNtpTabHelper::OnRestoreSession` |
| `switchToOpenTab` | `[tabId, windowId]` — finds the matching `Browser`/`TabStripModel` entry by id and calls `ActivateTabAt` + `window()->Activate()`, ported from `RemoteNtpTabHelper::OnSwitchToTab` |
...
| `recentlyClosedChanged` | — | `sessions::TabRestoreServiceObserver::TabRestoreServiceChanged` — React refetches `requestRecentlyClosed` |
| `openTabsChanged` | — | The handler's private `OpenTabsWatcher` (a `BrowserListObserver` + `TabStripModelObserver` attached to every open window of the profile, mirroring `RemoteNtpServiceImpl`'s own wiring) fires on any tab open/close/switch/move across any window — React refetches `requestOpenTabs` |
...
| [`src/custom/common/webui_url_constants.h`](../../src/custom/common/webui_url_constants.h) | `kChromeUISidebarHost`, `kChromeUISidebar{Bookmarks,History,Rss,Notes,NtpSettings,Agent,RecentlyClosed}URL` |
...
8. Open several tabs across two separate browser windows. Click the **Recently Closed** button (undo icon). **Open Tabs** should list every tab from both windows, with a dot marking each window's active tab. Switch tabs or open/close a tab in either window — the list updates live. Click a row from the *other* window — that window should activate and the tab should become selected. Close a tab, then close an entire window — both should appear under **Recently Closed** (the window entry shows a tab-count badge). Click a closed-tab row to restore it as a new tab in the window hosting the sidebar; click a closed-window row to restore the whole window.
