...
|---|---|---|
| Docked | `CREATE_AS_DOCKED` | Panels pack from the bottom-right of the work area leftward. Auto-shrinks panel widths once the row would overflow. Default mode, and (until v1.8.22) the only mode reachable from the extension API. |
| Detached | `CREATE_AS_DETACHED` | Free-floating at a user-chosen position. **Fixed 2026-08-16 (v1.8.22)**: `chrome.windows.create({type:'detached_panel'})` now actually creates a detached panel. `kDetachedPanel` was never really missing from `windows.json` — this fork's own patch already re-added it — the gap was a missing consuming `case` for it in the `CreateType` switch in `chrome/browser/extensions/api/tabs/tabs_api_non_android.cc` (the file itself was also renamed from `tabs_api.cc` upstream, hence the stale `TODO(panels-revival)` comment's file reference). Added the case alongside the existing `kPanel` one, threading a new `panel_create_mode` local through to the `CreatePanel()` call instead of the previously-hardcoded `CREATE_AS_DOCKED`. |
| Stacked | (no live-drag API; drag-driven, session restore excepted) | Vertical column of multiple panels. Created interactively by `PanelManager::CreateStack` when one panel is dragged onto another; there's still no extension API for "create me a stack" live. Session restore (v1.8.20+) does programmatically recreate a stack via the same `CreateStack`/`MovePanelToCollection` primitives — see "Known limitations" below. Stacks share a native window via `PanelStackView`. |
...
| ~~**Detached panels not reachable via extension API**~~ **Fixed 2026-08-16 (v1.8.22).** `kDetachedPanel` was never actually missing from `windows.json` (this fork's own patch already re-added it) — the real gap was a missing consuming `case` for it in `tabs_api_non_android.cc`'s `CreateType` switch. Added, mirroring the existing `kPanel` case but selecting `PanelManager::CREATE_AS_DETACHED` via a new `panel_create_mode` local (previously hardcoded to `CREATE_AS_DOCKED` at the single `CreatePanel()` call site both cases now share). Was a separate gap from session restore below despite both being tracked under the same historical `TODO(panels-revival)` comment. |
| ~~**Session restore restores as detached**~~ **Fixed 2026-08-15 (v1.8.20).** Turned out to be more fundamental than originally described: `PanelManager::SaveSessionToPrefs`/`RestoreSessionFromPrefs`/`RegisterPrefs` were entirely dead code — the pref was never registered with `local_state()`, and none of the three was ever called from anywhere in the browser lifecycle, so panels didn't persist across a restart *at all* (not "restores as detached," just didn't restore). Now wired up: `RegisterPrefs` is called from `custom_prefs.cc`'s `RegisterLocalState`; `SaveSessionToPrefs` fires incrementally from `CreatePanel`/`OnPanelClosed` (matching the function's own original design-intent comment) instead of a shutdown hook; `RestoreSessionFromPrefs` runs once at startup from `CustomBrowserMainExtraPartsProfiles::PostProfileInit`. The saved format gained `collection_type` (`detached`/`docked`/`stacked`) and, for stacked panels, a save-local `stack_id` grouping key — both optional fields, so pre-fix saved sessions still parse and restore detached exactly as before. Docked panels now restore docked; stacked panels restore into the same stack, in the same top-to-bottom order, via `CreateStack`/`MovePanelToCollection` (the same primitives live drag-to-stack already used) — a genuinely new capability, not just a state-loss fix, since there was previously no programmatic way to recreate a stack at all. |
| **Stacked panels — still drag-only for live creation** | Session restore (above) can now recreate a stack programmatically, but there's still no *extension API* for "create me a stack" during a live session — that remains drag-only. Stack-teardown reset state (the `secondary_stack_window_ = nullptr` family) is also still commented out — won't bite until a user actually stacks panels. |
