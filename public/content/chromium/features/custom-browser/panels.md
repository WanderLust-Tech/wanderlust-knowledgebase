...
| Detached | `CREATE_AS_DETACHED` | Free-floating at a user-chosen position. **Correction**: `kDetachedPanel` is not actually missing from `windows.json` — this fork's own patch already re-adds it. What's missing is a consuming `case` for it in the `CreateType` switch in `chrome/browser/extensions/api/tabs/tabs_api_non_android.cc` (the file itself was also renamed from `tabs_api.cc` upstream) — the `TODO(panels-revival)` comment there is stale in wording but the underlying gap (extension JS can't request a detached panel) is real. This is a separate, still-open, extension-API-only gap — unrelated to session restore (see below). |
...
| **Detached panels not reachable via extension API** | `kDetachedPanel` is already back in `windows.json` (this fork's own patch re-added it) — it's the consuming `case` in `tabs_api_non_android.cc`'s `CreateType` switch that's still missing, not the schema value. Separate, still-open gap; unrelated to session restore below despite both being tracked under the same historical `TODO(panels-revival)` comment. |
